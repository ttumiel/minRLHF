...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.register_buffer("causal_mask", torch.tril(torch.ones(config.block_size, config.block_size))
                                     .view(1, 1, config.block_size, config.block_size) == 0)
    ...
    def forward(self, x, attention_mask=None):
        ...
        if attention_mask is not None:
            mask = mask | attention_mask[:, None, None, :] # (B, 1, 1, T) Broadcast over heads and attention outputs

        att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))
        att = att.masked_fill(mask, torch.finfo(att.dtype).min) # having a padding mask with causal attention may completely mask out some tokens, resulting in nans in the softmax. we could also, multiply the softmax output by (1-mask) instead
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        assert len(keys) == len([k for k in sd if not k.endswith('.attn.causal_mask')])
        for k in keys:
    ...
    def forward(self, idx, targets=None, attention_mask=None):
        ...
        x = self.transformer(idx, attention_mask=attention_mask)
    ...
    def log_probs(self, idx, tgt, attention_mask=None):
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, do_sample=False, top_k=None, attention_mask=None, stop_at=None):
        ...
        for _ in range(max_new_tokens):
            ...
            if attention_mask is not None:
                ...
                if attention_mask.size(1) >= self.block_size:
